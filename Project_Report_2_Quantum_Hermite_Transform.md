...
## Status
Core verification complete (6 sub-projects); extension roadmap in progress — see Section 5 below and the companion repository `qiskit-quantum-hermite-transform`
...
Most literature on quantum algorithms I encounter in the course of my
post-quantum cryptography work is either decades old (Shor, Grover,
Simon) or purely asymptotic, with no implementation at any scale. In
June 2026, Jain, Iyer, Somma, Bao and Jordan (Google Quantum AI /
Brookhaven National Laboratory / Northeastern University / UT Austin)
published *"Efficient Quantum Hermite Transform"* at STOC '26,
introducing a genuinely new algorithmic primitive — alongside the QFT
and amplitude amplification, by the authors' own framing — with a
provable, non-asymptotic-hand-waving core claim: that the quantum
harmonic oscillator (QHO) can be *fast-forwarded*, meaning its time
evolution for arbitrarily long durations can be applied with a circuit
of constant size. This claim was independently checkable, and recent
enough that no third-party verification of it existed yet. I undertook
this project to verify it myself rather than take it on citation alone,
and to explore what it implies for problems adjacent to my own research
area, particularly the Gaussian analogue of the Goldreich-Levin
algorithm — a direct generalization of a foundational cryptographic
result (hardcore predicates for one-way functions) that this paper's
authors give surprisingly little space to relative to its significance.

## 2. Objectives

1. Independently implement, in Qiskit, the paper's central technical
   result (Theorem 1/3/5: exponential fast-forwarding of the discrete
   QHO) and verify its correctness numerically, rather than accepting
   the published complexity bound on faith.
2. Verify that the discretized "Hermite states" the paper's transform
   maps into are genuine eigenstates of the discretized QHO Hamiltonian,
   to the precision the paper claims.
3. Implement and test the paper's stated cryptographically-relevant
   application — the Gaussian Goldreich-Levin algorithm — explicitly
   alongside its classical ancestor, to make the connection between this
   new primitive and foundational cryptography concrete rather than
   asserted.
4. Extend beyond the paper's own worked examples into two further
   applications the paper only gestures at (property testing for
   low-degree closeness; Hamiltonian simulation via the Jaynes-Cummings
   model) and into the genuinely multivariate regime, which is where the
   paper's actual query-complexity advantage claim (independent of the
   number of variables) becomes a meaningful, testable statement rather
   than a single-variable correctness check.

## 3. Methodology and Scope

All work in this project was implemented from first principles —
building the discrete QHO position/momentum operators, the fast-
forwarding operator factorization, and the Hermite-sampling machinery
directly from the paper's equations — rather than relying on any
pre-built library implementation (none exists publicly for this
specific, six-week-old result at the time of this project). Every
numerical claim below was independently computed and cross-checked
against classical linear algebra (exact matrix exponentiation,
direct diagonalization) run in parallel with the Qiskit circuit
execution, so that correctness could be verified rather than assumed.

This project is explicit, throughout, about the boundary between what
was verified exactly and what was simplified for tractability at the
small scale (4-9 qubits) used here — for instance, the paper's
polylogarithmic-depth Plancherel-Rotach-based state preparation is
substituted with Qiskit's `initialize` primitive (exact, but
exponentially costly), since the goal of this project was to verify the
mathematical properties and mechanisms the paper describes, not to
reproduce its full resource-optimal construction end to end.

## 4. Key Findings (Summary)

The full technical report, reproduced below this front matter, covers
all results in detail. Headline findings from the six sub-projects
completed:

| Sub-project | Verified result |
|---|---|
| QHO fast-forwarding | A single, fixed 7-gate circuit reproduces exact time evolution with fidelity >=0.999999 for evolution times up to 200 — versus an estimated ~11,900 steps a naive Trotter simulation would need at that scale. A real implementation bug (the paper's `\|t\|<=pi/2` precondition, missed on first pass) was caught specifically by testing across this full range rather than a handful of convenient values. |
| Hermite state verification | All six tested discretized Hermite states matched QHO eigenstates to fidelity 1.00000000, eigenvalues exact to below 2×10⁻¹³. |
| Boolean vs. Gaussian Goldreich-Levin | Both the classical/Boolean algorithm (via Fourier/Walsh-Hadamard sampling) and the paper's Gaussian analogue (via Hermite sampling) correctly recovered hidden heavy spectral coefficients to within 1-2% of their true weights — including a genuine "needle in a haystack" case (1 heavy coefficient among 10 noise terms, 64 possible locations). |
| Degree-closeness property testing | Correctly distinguished a genuinely low-degree function (PASS) from a high-degree one (FAIL) via a single Hermite-spectrum tail-mass estimate. |
| Jaynes-Cummings fast-forwarding | Confirmed the paper's own cited example: the Hamiltonian is 100% dense in the field's position basis but only 11.7% dense (exactly block-diagonal) in the Fock/Hermite basis, enabling closed-form fast-forwarding verified to fidelity 1.00000000. |
| Multivariate QHT | Recovered a hidden 3-variable multi-index (1 of 512 possible) as the #1 most probable outcome in ~1-2 shots — the sub-project specifically designed to make the paper's *n*-independent query-complexity claim concrete, since single-variable demos cannot test that property by construction. |

## 5. Extension Work Completed and Planned

**Completed**, beyond the paper's own explicit examples:
- The Boolean-vs-Gaussian Goldreich-Levin comparison (Section 4,
  sub-project 3) is, to my knowledge, not present in the source paper or
  any secondary literature on it — it was undertaken specifically to
  connect this new primitive back to foundational cryptography (hardcore
  predicates, one-way functions, pseudorandom generators) in a way the
  original paper does not itself develop.
- The multivariate query-advantage demonstration (sub-project 6) required
  independently resolving a genuine tensor-product/qubit-register
  ordering bug not covered by any worked example in the paper.

**Planned / in progress:**
- Exploring whether Hermite-sampling techniques have any bearing on
  distinguishing-problem formulations relevant to Learning-With-Errors-
  style Gaussian-noise hardness assumptions — a research question
  motivated by, but not addressed in, the source paper, and directly
  adjacent to my own RLWE cryptosystem work. This is presently at the
  question-formulation stage, not a result.
- Running at least one circuit from this project on real IBM hardware,
  extending the verification standard applied throughout (simulator
  correctness checked against classical ground truth) to actual noisy
  hardware.
- A parallel quantum-classical co-processing project (in progress, see
  `qiskit-quantum-classical-coprocessing`) applying the same
  resource-aware, independently-verified methodology used throughout
  this project to variational algorithms (VQE), circuit cutting, and
  Qiskit Runtime primitive workflows.

---

## Full Report

*(The complete report — background, prerequisites, the paper's core
theorems explained, all six verified sub-projects with full
methodology, and references — follows below, unedited from the
standalone version of this report.)*
